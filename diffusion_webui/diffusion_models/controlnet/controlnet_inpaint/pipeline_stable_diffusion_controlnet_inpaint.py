...
EXAMPLE_DOC_STRING = """
    Examples:
        ...
        >>> from ppdiffusers import StableDiffusionControlNetInpaintPipeline, ControlNetModel, UniPCMultistepScheduler
        ...
        ...     "runwayml/stable-diffusion-inpainting", controlnet=controlnet, paddle_dtype=paddle.float16
        ... )
        ...
        >>> # generate image
        >>> generator = paddle.manual_seed(0)
        >>> image = pipe(
...
def prepare_mask_and_masked_image(image, mask):
    ...
    if isinstance(image, paddle.Tensor):
        ...
        image = paddle.from_numpy(image).to(dtype=paddle.float32) / 127.5 - 1.0
...
class StableDiffusionControlNetInpaintPipeline(
    ...
    def prepare_mask_latents(
        ...
        if isinstance(generator, list):
            ...
            masked_image_latents = paddle.cat(masked_image_latents, dim=0)
        else:
        ...
        mask = paddle.cat([mask] * 2) if do_classifier_free_guidance else mask
        masked_image_latents = (
            paddle.cat([masked_image_latents] * 2)
    ...
    def __call__(
        ...
        image: Union[paddle.to_tensor, PIL.Image.Image] = None,
        control_image: Union[
            paddle.to_tensor,
            PIL.Image.Image,
            List[paddle.to_tensor],
            List[PIL.Image.Image],
        ] = None,
        mask_image: Union[paddle.to_tensor, PIL.Image.Image] = None,
        ...
        eta: float = 0.0,
        generator: Optional[
            Union[paddle.Generator, List[paddle.Generator]]
        ] = None,
        latents: Optional[paddle.to_tensor] = None,
        prompt_embeds: Optional[paddle.to_tensor] = None,
        negative_prompt_embeds: Optional[paddle.to_tensor] = None,
        output_type: Optional[str] = "pil",
        return_dict: bool = True,
        callback: Optional[
            Callable[[int, int, paddle.to_tensor], None]
        ] = None,
        ...
        Args:
            ...
            control_image (`paddle.to_tensor`, `PIL.Image.Image`, `List[paddle.to_tensor]` or `List[PIL.Image.Image]`):
                The ControlNet input condition. ControlNet uses this input condition to generate guidance to Unet. If
                the type is specified as `paddle.to_tensor`, it is passed to ControlNet as is. PIL.Image.Image` can
            ...
            latents (`paddle.to_tensor`, *optional*):
                Pre-generated noisy latents, sampled from a Gaussian distribution, to be used as inputs for image
                generation. Can be used to tweak the same generation with different prompts. If not provided, a latents
                tensor will ge generated by sampling using the supplied random `generator`.
            prompt_embeds (`paddle.to_tensor`, *optional*):
                Pre-generated text embeddings. Can be used to easily tweak text inputs, *e.g.* prompt weighting. If not
                provided, text embeddings will be generated from `prompt` input argument.
            negative_prompt_embeds (`paddle.to_tensor`, *optional*):
            ...
            callback (`Callable`, *optional*):
                ...
                called with the following arguments: `callback(step: int, timestep: int, latents: paddle.to_tensor)`.
            ...
            cross_attention_kwargs (`dict`, *optional*):
                ...
                `self.processor` in
                [ppdiffusers.cross_attention](https://github.com/huggingface/ppdiffusers/blob/main/src/ppdiffusers/models/cross_attention.py).
        ...
        if do_classifier_free_guidance:
            control_image = paddle.cat([control_image] * 2)
